...
const PROJECTS: ProjectCard[] = [
    {
        ...
        path: [
            "CMS/bible-education-centre-2.png",
            "CMS/bible-education-centre-3.png",
            "CMS/bible-education-centre-4.png",
            "CMS/bible-education-centre.png"
        ],
...
const ProjectCard = ({ project }: { project: ProjectCard }) => {
    const primaryImage = Array.isArray(project.path) ? project.path[0] : project.path;
    const additionalImages = Array.isArray(project.path) ? project.path.slice(1) : [];

    return (
        <Card sx={{ borderRadius: 4 }}>
            <CardActionArea>
                <a href={primaryImage} className="glightbox" data-gallery={`gallery-${project.id}`}>
                    <CardMedia
                        component="img"
                        height="200"
                        image={primaryImage}
...
export default function Hero() {
    useEffect(() => {
        ...
        const initializeLightbox = async () => {
            try {
                const GLightbox = (await import('glightbox')).default || (await import('glightbox'));
                lightbox = GLightbox({
                    selector: '.glightbox',
                    loop: true,
                });
